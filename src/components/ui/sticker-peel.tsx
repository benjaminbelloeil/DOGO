...
// Source: React Bits — StickerPeel (TypeScript + Tailwind). Draggable, peelable
// sticker built on GSAP Draggable.
...
const StickerPeel: React.FC<StickerPeelProps> = ({
  ...
  useEffect(() => {
    ...
    const parentBounds = target.parentElement;
    const selectorBounds = dragBounds
      ? document.querySelector<HTMLElement>(dragBounds)
      : null;
    const boundsEl = selectorBounds ?? parentBounds;
    ...
    const handleResize = () => {
      if (draggableInstanceRef.current) {
        ...
        if (targetRect.left < boundsRect.left) {
          newX += boundsRect.left - targetRect.left;
        } else if (targetRect.right > boundsRect.right) {
          newX -= targetRect.right - boundsRect.right;
        }

        if (targetRect.top < boundsRect.top) {
          newY += boundsRect.top - targetRect.top;
        } else if (targetRect.bottom > boundsRect.bottom) {
          newY -= targetRect.bottom - boundsRect.bottom;
        }
